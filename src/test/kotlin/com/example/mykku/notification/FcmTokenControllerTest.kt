...
class FcmTokenControllerTest : BaseControllerTest() {
    ...
    fun `registerToken - 이미 존재하는 deviceId는 토큰을 갱신한다`() {
        ...
        val existingToken = fcmTokenRepository.save(
            FcmToken.create(
                member = member,
                token = "old_token",
                deviceId = "device_001",
        ...
        val updatedToken = fcmTokenRepository.findById(existingToken.id!!).get()
    ...
    fun `deleteToken - 정상적으로 FCM 토큰을 삭제한다`() {
        ...
        assert(!fcmTokenRepository.existsByMemberAndDeviceId(member, "device_001"))
    }
