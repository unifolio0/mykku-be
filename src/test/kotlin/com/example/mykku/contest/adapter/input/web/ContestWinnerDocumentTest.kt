...
class ContestWinnerDocumentTest : BaseDocumentTest() {
    ...
    inner class SetWinners {
        ...
        fun `성공`() {
            ...
            val result = SetContestWinnersResult(
                contestId = contestId,
            ...
            `when`(setContestWinnersUseCase.execute(any())).thenReturn(result)
        ...
        fun `콘테스트 미종료`() {
            ...
            val request = SetContestWinnersRequest(
                winners = listOf(
                    WinnerSelectionRequest(participationId = 1L, winnerRank = 1)
                )
            )

            `when`(setContestWinnersUseCase.execute(any()))
        ...
        fun `중복 순위`() {
            ...
            val request = SetContestWinnersRequest(
                winners = listOf(
                    WinnerSelectionRequest(participationId = 1L, winnerRank = 1),
                    WinnerSelectionRequest(participationId = 2L, winnerRank = 1)
                )
            )
