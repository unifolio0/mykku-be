package com.example.mykku.preference.adapter.output.persistence

import com.example.mykku.BaseRepositoryTest
import com.example.mykku.preference.application.port.output.GoodsPreferenceRepository
import com.example.mykku.preference.domain.entity.MemberGoodsPreference
import com.example.mykku.preference.domain.vo.GoodsType
import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.DisplayName
import org.junit.jupiter.api.Nested
import org.junit.jupiter.api.Test
import org.springframework.beans.factory.annotation.Autowired

@DisplayName("GoodsPreferenceRepositoryAdapter 통합 테스트")
class GoodsPreferenceRepositoryAdapterTest : BaseRepositoryTest() {

    @Autowired
    private lateinit var goodsPreferenceRepository: GoodsPreferenceRepository

    @Nested
    @DisplayName("saveAll 메서드")
    inner class SaveAll {

        @Test
        @DisplayName("굿즈 선호도 목록을 저장한다")
        fun `굿즈 선호도 목록 저장 - 정상 케이스`() {
            val member = createAndSaveMember(id = "member1")
            val preferences = listOf(
                MemberGoodsPreference.create(member.id, GoodsType.PHOTOCARD_HOLDER),
                MemberGoodsPreference.create(member.id, GoodsType.ITABAG)
            )

            val saved = goodsPreferenceRepository.saveAll(preferences)

            assertThat(saved).hasSize(2)
            assertThat(saved[0].id.value).isNotEqualTo(0L)
            assertThat(saved[1].id.value).isNotEqualTo(0L)
            assertThat(saved.map { it.goodsType }).containsExactlyInAnyOrder(
                GoodsType.PHOTOCARD_HOLDER,
                GoodsType.ITABAG
            )
        }

        @Test
        @DisplayName("빈 목록을 저장하면 빈 목록을 반환한다")
        fun `빈 목록 저장 - 빈 결과 반환`() {
            val saved = goodsPreferenceRepository.saveAll(emptyList())

            assertThat(saved).isEmpty()
        }

        @Test
        @DisplayName("모든 굿즈 타입을 저장할 수 있다")
        fun `모든 굿즈 타입 저장 - 정상 케이스`() {
            val member = createAndSaveMember(id = "member1")
            val preferences = GoodsType.entries.map { goodsType ->
                MemberGoodsPreference.create(member.id, goodsType)
            }

            val saved = goodsPreferenceRepository.saveAll(preferences)

            assertThat(saved).hasSize(GoodsType.entries.size)
            assertThat(saved.map { it.goodsType }).containsExactlyInAnyOrderElementsOf(GoodsType.entries)
        }
    }

    @Nested
    @DisplayName("findByMemberId 메서드")
    inner class FindByMemberId {

        @Test
        @DisplayName("회원의 굿즈 선호도 목록을 조회한다")
        fun `회원별 굿즈 선호도 조회 - 정상 케이스`() {
            val member = createAndSaveMember(id = "member1")
            val preferences = listOf(
                MemberGoodsPreference.create(member.id, GoodsType.PHOTOCARD_HOLDER),
                MemberGoodsPreference.create(member.id, GoodsType.DESK_TERIOR),
                MemberGoodsPreference.create(member.id, GoodsType.LIGHT_STICK_DECO)
            )
            goodsPreferenceRepository.saveAll(preferences)

            val found = goodsPreferenceRepository.findByMemberId(member.id)

            assertThat(found).hasSize(3)
            assertThat(found.map { it.goodsType }).containsExactlyInAnyOrder(
                GoodsType.PHOTOCARD_HOLDER,
                GoodsType.DESK_TERIOR,
                GoodsType.LIGHT_STICK_DECO
            )
        }

        @Test
        @DisplayName("선호도가 없는 회원 조회 시 빈 목록을 반환한다")
        fun `선호도 없는 회원 조회 - 빈 목록 반환`() {
            val member = createAndSaveMember(id = "member1")

            val found = goodsPreferenceRepository.findByMemberId(member.id)

            assertThat(found).isEmpty()
        }

        @Test
        @DisplayName("다른 회원의 선호도는 조회되지 않는다")
        fun `다른 회원 선호도 조회 불가 - 격리 검증`() {
            val member1 = createAndSaveMember(id = "member1", email = "member1@test.com")
            val member2 = createAndSaveMember(id = "member2", email = "member2@test.com")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member1.id, GoodsType.PHOTOCARD_HOLDER)
            ))
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member2.id, GoodsType.ITABAG)
            ))

            val found = goodsPreferenceRepository.findByMemberId(member1.id)

            assertThat(found).hasSize(1)
            assertThat(found[0].goodsType).isEqualTo(GoodsType.PHOTOCARD_HOLDER)
        }
    }

    @Nested
    @DisplayName("deleteByMemberId 메서드")
    inner class DeleteByMemberId {

        @Test
        @DisplayName("회원의 모든 굿즈 선호도를 삭제한다")
        fun `회원별 굿즈 선호도 삭제 - 정상 케이스`() {
            val member = createAndSaveMember(id = "member1")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member.id, GoodsType.PHOTOCARD_HOLDER),
                MemberGoodsPreference.create(member.id, GoodsType.ITABAG)
            ))

            goodsPreferenceRepository.deleteByMemberId(member.id)

            val found = goodsPreferenceRepository.findByMemberId(member.id)
            assertThat(found).isEmpty()
        }

        @Test
        @DisplayName("선호도가 없는 회원 삭제 시 예외가 발생하지 않는다")
        fun `선호도 없는 회원 삭제 - 예외 없음`() {
            val member = createAndSaveMember(id = "member1")

            goodsPreferenceRepository.deleteByMemberId(member.id)

            val found = goodsPreferenceRepository.findByMemberId(member.id)
            assertThat(found).isEmpty()
        }

        @Test
        @DisplayName("다른 회원의 선호도는 삭제되지 않는다")
        fun `다른 회원 선호도 삭제 불가 - 격리 검증`() {
            val member1 = createAndSaveMember(id = "member1", email = "member1@test.com")
            val member2 = createAndSaveMember(id = "member2", email = "member2@test.com")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member1.id, GoodsType.PHOTOCARD_HOLDER)
            ))
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member2.id, GoodsType.ITABAG)
            ))

            goodsPreferenceRepository.deleteByMemberId(member1.id)

            val member1Preferences = goodsPreferenceRepository.findByMemberId(member1.id)
            val member2Preferences = goodsPreferenceRepository.findByMemberId(member2.id)
            assertThat(member1Preferences).isEmpty()
            assertThat(member2Preferences).hasSize(1)
        }
    }

    @Nested
    @DisplayName("existsByMemberIdAndGoodsType 메서드")
    inner class ExistsByMemberIdAndGoodsType {

        @Test
        @DisplayName("회원의 특정 굿즈 선호도가 존재하면 true를 반환한다")
        fun `선호도 존재 확인 - true 반환`() {
            val member = createAndSaveMember(id = "member1")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member.id, GoodsType.PHOTOCARD_HOLDER)
            ))

            val exists = goodsPreferenceRepository.existsByMemberIdAndGoodsType(member.id, GoodsType.PHOTOCARD_HOLDER)

            assertThat(exists).isTrue()
        }

        @Test
        @DisplayName("회원의 특정 굿즈 선호도가 없으면 false를 반환한다")
        fun `선호도 미존재 확인 - false 반환`() {
            val member = createAndSaveMember(id = "member1")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member.id, GoodsType.PHOTOCARD_HOLDER)
            ))

            val exists = goodsPreferenceRepository.existsByMemberIdAndGoodsType(member.id, GoodsType.ITABAG)

            assertThat(exists).isFalse()
        }

        @Test
        @DisplayName("선호도가 없는 회원 확인 시 false를 반환한다")
        fun `선호도 없는 회원 확인 - false 반환`() {
            val member = createAndSaveMember(id = "member1")

            val exists = goodsPreferenceRepository.existsByMemberIdAndGoodsType(member.id, GoodsType.PHOTOCARD_HOLDER)

            assertThat(exists).isFalse()
        }

        @Test
        @DisplayName("다른 회원의 선호도는 확인되지 않는다")
        fun `다른 회원 선호도 확인 불가 - 격리 검증`() {
            val member1 = createAndSaveMember(id = "member1", email = "member1@test.com")
            val member2 = createAndSaveMember(id = "member2", email = "member2@test.com")
            goodsPreferenceRepository.saveAll(listOf(
                MemberGoodsPreference.create(member1.id, GoodsType.PHOTOCARD_HOLDER)
            ))

            val existsForMember2 = goodsPreferenceRepository.existsByMemberIdAndGoodsType(member2.id, GoodsType.PHOTOCARD_HOLDER)

            assertThat(existsForMember2).isFalse()
        }
    }
}
