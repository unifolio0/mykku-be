...
class MemberContestDocumentTest : BaseDocumentTest() {
    ...
    fun `내가 참여한 콘테스트 목록 조회`() {
        ...
        `when`(contestService.getMyParticipatedContests(any(), any(), any())).thenReturn(response)
    ...
    fun `내가 참여한 콘테스트 목록 조회 - 빈 목록`() {
        ...
        val documentFilter = document("member/contests", "empty")
            .request(
                request()
                    .tag(Tag.MEMBER_API)
                    .summary("내가 참여한 콘테스트 목록 조회 - 빈 목록")
                    .description("참여한 콘테스트가 없는 경우 빈 목록을 반환합니다.")
                    .queryParameter(
                        parameterWithName("page").description("페이지 번호 (0부터 시작, 기본값: 0)").optional(),
                        parameterWithName("size").description("페이지 크기 (기본값: 20)").optional()
                    )
