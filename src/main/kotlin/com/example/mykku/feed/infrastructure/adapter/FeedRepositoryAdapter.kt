...
import com.example.mykku.feed.infrastructure.persistence.mapper.FeedMapper
...
class FeedRepositoryAdapter(
    ...
    override fun save(feed: FeedDomain): FeedDomain {
        throw UnsupportedOperationException("Pure domain save not yet implemented. Use createFeed() for legacy compatibility.")
    }

    override fun findById(id: FeedId): FeedDomain? {
        return feedRepository.findById(id.value)
            .map { feedMapper.toDomain(it) }
            .orElse(null)
    }

    override fun findByAuthorId(authorId: MemberId): List<FeedDomain> {
        return feedRepository.findByMemberId(authorId.value)
            .map { feedMapper.toDomain(it) }
    }
    ...
    override fun delete(feed: FeedDomain) {
        feed.id?.let { feedRepository.deleteById(it.value) }
            ?: throw IllegalArgumentException("Cannot delete feed without id")
    }
